...
import { 
  User, Department, Course, Classroom, Unit, TimetableEntry, AcademicSetting, DayOfWeek, TrainerSlotPreference, AvailabilityType 
} from '../types';
import { 
...
export default function TrainerDashboard({
  ...
  // Filter ONLY published timetable entries assigned to me
  const myClasses = timetableEntries.filter(e => e.trainerId === currentUser.id && e.isPublished);

  const getCellDetails = (day: DayOfWeek, slotId: number) => {
    const entry = myClasses.find(e => e.day === day && e.slotId === slotId);
            ...
            <div id="section-my-timetable">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 print:hidden">
                ...
                <div className="flex items-center gap-4 bg-white px-4 py-2.5 rounded-xl border border-slate-200">
                  <span className="text-xs text-slate-500 font-semibold">Total Hours: <span className="text-indigo-600 font-bold">{myClasses.length * 2} Hrs / Week</span></span>
              ...
              <div className="hidden print:flex print:flex-col print:items-center print:justify-center text-center mb-8 border-b-2 border-slate-800 pb-4">
                ...
                <p className="text-xs uppercase font-mono tracking-wider text-slate-500 mt-1">PERSONAL EDUCATOR TIMETABLE SCHEDULE</p>
                <div className="mt-4 flex justify-between text-xs font-semibold text-slate-700 max-w-xl mx-auto">
                  <span>TRAINER: {currentUser.name}</span>
              ...
              <div className="bg-white rounded-3xl border border-slate-200 shadow-xs overflow-hidden print:border-none print:shadow-none">
                <div className="overflow-auto max-h-[70vh]">
                  <table className="min-w-full divide-y divide-slate-100 print:divide-slate-800">
                      ...
                      {daysOfWeek.map(day => (
                        <tr key={day} className="hover:bg-slate-50/20 transition-colors">
                          ...
                          {timeSlots.map(ts => {
                            ...
                            return (
                              <td key={ts.id} className="px-4 py-4 align-top w-1/5 min-w-[155px] relative border-r border-slate-100 last:border-r-0 print:border border-slate-300">
                                {details ? (
                                  <div className="p-2.5 rounded-xl border border-indigo-100 bg-indigo-50/30 text-slate-800">
                                    <div className="flex items-center justify-between gap-1 mb-2">
                                      <span className="font-mono font-bold text-[9px] uppercase bg-slate-100 border border-slate-200 py-0.5 px-1 rounded text-slate-700">
                                        {details.unit?.code}
                                      </span>
                                    ...
                                    <span className="text-[9px] text-slate-400 block mt-1 italic leading-none">
                                      Program: {details.course?.code} ({details.entry.semesterName})
                                    </span>
                                  </div>
                                ) : (
