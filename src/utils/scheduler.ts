...
export function autoGenerateDepartmentTimetable(
  ...
  // 1. Identify courses & units to schedule in this department & semester
  const deptCourses = state.courses.filter(c => c.departmentId === departmentId);
  const deptCourseIds = new Set(deptCourses.map(c => c.id));

  // 2. Keep entries of OTHER departments, OTHER semesters, or OTHER courses not in this scheduling batch
  const externalEntries = state.entries.filter(
    e => !(e.departmentId === departmentId && e.semesterName === semesterName && deptCourseIds.has(e.courseId))
  );
